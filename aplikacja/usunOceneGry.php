<?php
    ...
    if(isset($_POST['Nazwa']))
    {
        ...
        try
        {
            ...
            {
                if($link->query("DELETE FROM OcenaGry WHERE idog = '$nazwa'"))
                {
...
<html>
    ...
    <body>
    <h1>
        Wybierz grę, której ocenę chcesz usunąć
    </h1>
        <div class="container">
            <form method="POST">
                <div class="row">
                    ...
                    <div class="col-75">
                        ...
                        try
                        {
                            ...
                            {
                                $u_id = $_SESSION['idu'];
                                if($result = $link->query("SELECT Nazwa, idog FROM Gra JOIN OcenaGry ON(idg = g_id) WHERE u_id = '$u_id' ORDER BY Nazwa"))
                                {
                                    while($row = $result->fetch_assoc())
                                    {
                                        echo '<option value="'.$row['idog'].'">'.$row['Nazwa'].'</option>';
                                    }
                                }
                                else
